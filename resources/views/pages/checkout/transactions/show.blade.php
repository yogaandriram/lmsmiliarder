...
@php
  ...
  $expires = $selected && $selected->expires_at ? $selected->expires_at : ($selected && optional($selected->transaction_time)?$selected->transaction_time->copy()->addHours(24):null);
@endphp

<div class="space-y-8">
  <div class="flex items-center justify-between">
    <h2 class="text-3xl md:text-4xl font-bold">Semua History Transaksi</h2>
    <a href="{{ route('member.transactions.index') }}" class="px-3 py-2 rounded bg-white/10 text-white/80 hover:bg-white/20">Kembali ke Dashboard Member</a>
  </div>
