...
<div class="max-w-5xl mx-auto space-y-6">
  <div class="flex items-center justify-between">
    ...
    <x-ui.btn-secondary href="javascript:history.back()" icon="fa-solid fa-arrow-left">Kembali</x-ui.btn-secondary>
  </div>

  <div class="grid grid-cols-1 place-items-center gap-6">
    <div class="w-full max-w-3xl glass p-6 rounded-lg">
      @if(session('error'))
        <div class="glass p-3 mb-3 rounded text-red-300">{{ session('error') }}</div>
      @endif
      ...
      <div class="border-t border-white/10 mt-4 pt-4">
          ...
          if ($couponCode !== '') {
            ...
            if (!$cp) {
              ...
              if (!is_null($cp->usage_limit) && $used >= (int)$cp->usage_limit) {
                ...
                if ($cp->discount_type === 'percentage') { $discountPreview = min((float)$ebook->price, ((float)$ebook->price * (float)$cp->discount_value) / 100.0); }
                else { $discountPreview = min((float)$ebook->price, (float)$cp->discount_value); }
                $couponMsg = 'Kupon diterapkan.';
              }
            }
          }
          $finalPreview = max(0.0, (float)$ebook->price - (float)$discountPreview);
        @endphp
        <div class="flex items-center justify-between">
        ...
        <div class="mt-4">
          ...
          @endphp
          @if($accounts->count())
            <div id="bank_cards_ebook" class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <input type="hidden" id="selected_bank_ebook" value="{{ $initialBank > 0 ? $initialBank : '' }}">
      ...
      @if(Auth::check())
      <form method="POST" action="{{ route('member.checkout.ebook.purchase', $ebook) }}" class="mt-6" id="checkout_form_ebook">
        @csrf
        @if($couponCode)
          <input type="hidden" name="coupon_code" value="{{ $couponCode }}">
        @endif
        <input type="hidden" name="bank_id" id="bank_id_ebook" value="{{ $initialBank > 0 ? $initialBank : '' }}">
        <x-ui.btn-primary type="submit" class="w-full justify-center" icon="fa-solid fa-credit-card">Bayar Sekarang</x-ui.btn-primary>
      </form>
      @else
        <div class="mt-6">
          <x-ui.btn-primary type="button" id="trigger_login_prompt_ebook" class="w-full justify-center" icon="fa-solid fa-credit-card">Bayar Sekarang</x-ui.btn-primary>
        </div>
      @endif
    </div>
...
<div id="login_prompt_modal_ebook" class="fixed inset-0 w-screen h-screen bg-black/60 z-50 hidden">
  <div class="glass p-6 rounded w-full max-w-md relative">
    <button type="button" id="close_login_prompt_ebook" class="absolute top-2 right-2 text-white/80 hover:text-yellow-400">
...
<script>
(function(){
  var btn = document.getElementById('trigger_login_prompt_ebook');
  var modal = document.getElementById('login_prompt_modal_ebook');
  var close = document.getElementById('close_login_prompt_ebook');
  var bankGrid = document.getElementById('bank_cards_ebook');
  var selHidden = document.getElementById('selected_bank_ebook');
  var bankIdField = document.getElementById('bank_id_ebook');
  var applyForm = document.querySelector('form[method="GET"]');
  var checkoutForm = document.getElementById('checkout_form_ebook');
