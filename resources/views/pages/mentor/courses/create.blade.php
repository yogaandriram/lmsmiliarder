...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-semibold">Buat Kursus Baru</h2>
            <p class="text-white/70 mt-1">Buat kursus menarik untuk dibagikan kepada siswa</p>
        </div>
    ...
    <div class="glass p-6 rounded-lg">
        <form method="POST" action="{{ route('mentor.courses.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <x-ui.crud.input label="Judul Kursus *" name="title" :value="old('title')" required variant="glass" />

                <x-ui.crud.textarea label="Deskripsi *" name="description" variant="glass">{{ old('description') }}</x-ui.crud.textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input id="price_display_create" type="text" inputmode="numeric" autocomplete="off"
                               class="w-full p-2 rounded bg-white/10 border border-white/20 text-white"
                               value="{{ old('price') !== null ? number_format((int)old('price'), 0, ',', '.') : '0' }}"
                               placeholder="0" />
                        <input type="hidden" name="price" id="price_value_create" value="{{ (int)old('price', 0) }}" />
                        @error('price')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <x-ui.crud.input label="URL Video Perkenalan" name="intro_video_url" type="url" :value="old('intro_video_url')" variant="glass" placeholder="https://youtube.com/..." />
                    </div>
                    <div>
                        <x-ui.crud.input label="Thumbnail (max 2 MB)" name="thumbnail" type="file" accept="image/*" variant="glass" />
                    </div>
                </div>
            </div>
