...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <x-ui.btn-secondary href="{{ route('mentor.discussions.index') }}" icon="fa-solid fa-arrow-left">Kembali</x-ui.btn-secondary>
  </div>

  <div class="glass p-0 rounded-lg overflow-hidden relative w-full">
    <div id="chat_messages" data-fetch-url="{{ route('mentor.discussions.chat.fetch', $group) }}" class="h-[70vh] overflow-y-auto p-4 space-y-3">
    ...
    <form id="chat_form" method="POST" action="{{ route('mentor.discussions.chat.post', $group) }}" class="flex items-center gap-3 p-4 border-t border-white/10 w-full" enctype="multipart/form-data">
      @csrf
  ...
  <script>
    (function(){
      ...
      setInterval(function(){
        var url = box ? box.getAttribute('data-fetch-url') : '';
        if(!url) return;
        fetch(url)
          ...
          .then(function(d){
            ...
            for(var i=0;i<d.messages.length;i++){
              ...
              html += `
                <div class="flex items-start gap-3">
                  <img src="${m.avatar || 'https://placehold.co/36x36'}" class="w-9 h-9 rounded-full object-cover" alt="">
                  <div class="max-w-[80%]">
                    <div class="text-sm text-white/70">${m.user} • ${m.time}</div>
                    <div class="p-3 bg-white/10 rounded">${(m.content || '').replace(/\n/g,'<br>')}</div>
                    ${m.file_url ? (m.mime_type && m.mime_type.indexOf('image') === 0 ? `<div class="mt-2"><img src="${m.file_url}" class="max-w-xs rounded" /></div>` : `<div class="mt-2"><a href="${m.file_url}" target="_blank" class="underline">${m.original_name || 'Lampiran'}</a></div>`) : ''}
                  </div>
                </div>`;
            }
            box.innerHTML = html; scrollBottom();
          });
      }, 5000);
    })();
  </script>
