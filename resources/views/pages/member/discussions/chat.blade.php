...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <x-ui.btn-secondary href="{{ route('member.discussions.index') }}" icon="fa-solid fa-arrow-left">Kembali</x-ui.btn-secondary>
  </div>

  <div class="glass p-0 rounded-lg overflow-hidden relative w-full">
    <div id="chat_messages" data-fetch-url="{{ route('member.discussions.chat.fetch', $group) }}" class="h-[70vh] overflow-y-auto p-4 space-y-3">
      @foreach($messages as $m)
        <div class="flex items-start gap-3">
          <img src="{{ $m->user->avatar_url ?? 'https://placehold.co/36x36' }}" class="w-9 h-9 rounded-full object-cover" alt="">
          <div class="max-w-[80%]">
            <div class="text-sm text-white/70">{{ $m->user->name }} • {{ optional($m->created_at)->format('H:i') }}</div>
            <div class="p-3 bg-white/10 rounded">{!! nl2br(e($m->content)) !!}</div>
            @if($m->file_url)
              @php $mime = $m->mime_type ?? ''; @endphp
              <div class="mt-2">
                @if(\Illuminate\Support\Str::startsWith($mime, 'image/'))
                  <img src="{{ $m->file_url }}" alt="lampiran" class="max-w-xs rounded border border-white/10">
                @else
                  <a href="{{ $m->file_url }}" target="_blank" class="underline text-yellow-300">{{ $m->original_name ?? 'Lampiran' }}</a>
                @endif
              </div>
            @endif
          </div>
    ...
    <form id="chat_form" method="POST" action="{{ route('member.discussions.chat.post', $group) }}" class="flex items-center gap-3 p-4 border-t border-white/10 w-full" enctype="multipart/form-data">
      @csrf
      <div class="flex-1 min-w-0">
        <x-ui.crud.input name="content" id="chat_input" placeholder="Ketik pesan..." class="w-full" />
      </div>
      <input type="file" name="file" id="chat_attachment" class="hidden" accept="image/*,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" />
...
(function(){
  ...
  function bindPicker(){ if(pickerListenerBound) return; var p=document.getElementById('emoji_picker'); if(p && input){ p.addEventListener('emoji-click', function(e){ input.value=(input.value||'') + (e.detail && e.detail.unicode ? e.detail.unicode : ''); input.focus(); if(panel){ panel.style.display='none'; emojiOpen=false; emojiBtn && emojiBtn.setAttribute('aria-expanded','false'); } }); pickerListenerBound = true; } }
