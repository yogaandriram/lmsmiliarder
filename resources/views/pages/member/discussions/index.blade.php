  ...
  <div class="glass p-6 rounded-lg">
        ...
        @foreach($groups as $group)
          <div class="glass p-5 rounded-lg flex flex-col gap-3">
            ...
            <div class="flex justify-between items-center mt-2">
              <div class="text-sm text-white/70">Peserta: <span class="font-semibold">{{ $group->course->enrollments_count }}</span></div>
              <x-ui.btn-secondary href="{{ route('member.discussions.chat', $group) }}" size="sm" icon="fa-solid fa-comments">Selengkapnya</x-ui.btn-secondary>
            </div>
          </div>
        @endforeach
      </div>
    @else
      <div class="text-center py-10 text-white/60">
        ...
        <div>Belum ada grup diskusi dari kursus yang Anda ikuti.</div>
      </div>
    @endif
  </div>
</div>
